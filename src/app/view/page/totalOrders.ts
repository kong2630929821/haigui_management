...
export class TotalOrder extends Widget {
    ...
    public showOrderBySid(e:any) {
        ...
        const timeDom = getRealNode(e.node).parentNode.children[5].children;
        const start = Date.parse(timeDom[0].value);
        const tail = Date.parse(timeDom[1].value);
        const timeType = timeDom[2].options.selectedIndex + 1;
    ...
    public showOrderByExportState(e:any) {
        ...
        // 根据时间筛选
    public showOrderByTime(e:any) {
        const timeDom = getRealNode(e.node).parentNode.children;
        const start = Date.parse(timeDom[0].value);
        const tail = Date.parse(timeDom[1].value);
        const otherDom = getRealNode(e.node).parentNode.parentNode.children;
        const supplierId = Number(otherDom[2].value);
        const orderType = otherDom[3].options.selectedIndex + 1;
        const exportType = Number(otherDom[4].options.selectedIndex);
        const timeType = timeDom[2].options.selectedIndex + 1;
        const argsList = [supplierId,orderType,exportType,start,tail,timeType];
        this.showAllOrder(argsList).then((orderList) => {
            // 显示订单
